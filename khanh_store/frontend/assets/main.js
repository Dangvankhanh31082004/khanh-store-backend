...
function updateHeaderAuthStatus() {
    ...
    if (user && loginBtn) {
        ...
        // Chỉ hiển thị link Admin cho các vai trò sau (case-insensitive)
        const allowedAdminRoles = ['ADMIN', 'OWNER', 'MANAGER', 'STAFF'];
        const userRole = (user.role || user.role_name || '').toString().toUpperCase();
        const showAdminLink = allowedAdminRoles.includes(userRole);
        ...
        parentLi.innerHTML = `
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2 rounded-3">
                ...
                ${showAdminLink ? `<li><hr class="dropdown-divider"></li><li><a class="dropdown-item py-2 fw-bold text-primary" href="admin/index.html"><i class="bi bi-speedometer2 me-2"></i> Trang Quản trị</a></li>` : ''}
...
function getProductImageUrl(imagePath) {
    ...
    if (imagePath.startsWith('/uploads/')) {
        const host = window.API_HOST || window.location.origin;
